...
def index(request):
    ...
    Janhavi = {'allProds': allProds}
    return render(request, 'shop/index.html', Janhavi)
...
def search(request):
    ...
    for cat in cats:
        ...
        if len(prod) != 0:
            allProds.append([prod, range(1, nSlides), nSlides])
    Janhavi = {'allProds': allProds, "msg": ""}
    if len(allProds) == 0 or len(query) < 3:
        Janhavi = {'msg': "No item available. Please make sure to enter relevant search query"}
    return render(request, 'shop/search.html', Janhavi)
...
def checkout(request):
    if request.method == "POST":
        ...
        if 'onlinePay' in request.POST:
            ...
            Janhavi_dict['CHECKSUMHASH'] = Checksum.generate_checksum(Janhavi_dict, MERCHANT_KEY)
            return render(request, 'shop/paytm.html', {'Janhavi_dict': Janhavi_dict})
